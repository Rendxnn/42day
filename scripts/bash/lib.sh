...
ensure_api_dev_vars() {
  local root="$1" api_env="$1/apps/api/.dev.vars" api_env_example="$1/apps/api/.dev.vars.example"
  if [[ -f "$api_env" ]]; then return 0; fi
...
pnpm_exec() {
  if command -v pnpm >/dev/null 2>&1; then pnpm "$@"; return 0; fi
  if command -v corepack >/dev/null 2>&1 && corepack pnpm --version >/dev/null 2>&1; then corepack pnpm "$@"; return 0; fi
  if command -v npm >/dev/null 2>&1; then npm exec --yes pnpm@9.15.0 -- "$@"; return 0; fi
